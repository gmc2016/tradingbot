...
export default function App() {
  const { data, connected, startBot, stopBot, setMode, runNow, refreshNews, updateSettings } = useDashboard()
  ...
  // Show loading screen only until first response — returns fast now
  if (!data) {
    return (
  ...
  const { stats, open_trades = [], recent_trades = [], pairs = [], news = [], mode, bot_running, usdt_balance, config = {} } = data
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        padding: '4px 12px', display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0,
      }}>
        {[['⚡ Run cycle now', runNow], ['📰 Refresh news', refreshNews], ['⚙ Settings', () => setShowSettings(true)]].map(([label, fn]) => (
        ...
        <span style={{ marginLeft: 'auto', fontSize: 11, color: 'var(--text-3)' }}>
          Auto-scan every 5 min · Prices refresh every 1 min · News every 15 min
        </span>
        {data.last_update && (
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden', minHeight: 0 }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minHeight: 0 }}>
          <KpiBar stats={stats} openTrades={open_trades} maxPositions={config.max_positions || 3} />

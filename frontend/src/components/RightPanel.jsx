...
export default function RightPanel({ openTrades = [], pairs = [], news = [] }) {
  return (
    <div style={{
      ...
      <Section title={`Open positions (${openTrades.length})`}>
        {openTrades.length === 0
          ...
          : openTrades.map(t => <PositionCard key={t.id} trade={t} />)}
      </Section>

      {/* AI Sentiment */}
      <Section title="AI Sentiment">
        {pairs.slice(0, 6).map(p => (
          <SentimentRow key={p.symbol} symbol={p.symbol} score={p.sentiment ?? 50} />
        ))}
      </Section>
...
function Section({ title, children, flex }) {
  return (
    <div style={{
      ...
      ...(flex ? { flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, overflow: 'hidden' } : {}),
    }}>
      <div style={{ fontSize: 10, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: .5, fontWeight: 600, marginBottom: 8 }}>
        {title}
      </div>
      {children}
    </div>
...
function Tag({ label, bg, color }) {
  return (
    <span style={{ fontSize: 10, padding: '2px 6px', borderRadius: 10, background: bg, color, fontWeight: 500 }}>
      {label}
    </span>
  )
}
...
function SentimentRow({ symbol, score }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
      <span style={{ fontSize: 11, color: 'var(--text-2)', width: 32 }}>{coin}</span>
...
function NewsItem({ item }) {
  ...
  return (
    <div style={{ paddingBottom: 8, marginBottom: 8, borderBottom: '1px solid var(--border-dim)' }}>

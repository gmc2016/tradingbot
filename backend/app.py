...
app.config['SECRET_KEY'] = get_setting('flask_secret') or os.environ.get('FLASK_SECRET', 'changeme')
CORS(app, origins='*')
socketio = SocketIO(app, cors_allowed_origins='*', async_mode='eventlet')
scheduler = BackgroundScheduler()

# ── Live price stream via Binance WebSocket ───────────────────────────────────
_prices = {}  # { 'BTC/USDT': {'price': 75000, 'change': 1.5} }
_ws_thread = None

def start_price_stream():
    """Connect to Binance miniTicker stream for real-time prices."""
    global _ws_thread
    ...
    def run():
        ...
        streams   = '/'.join([p.replace('/','').lower() + '@miniTicker'
        ...
        def on_close(ws, *args):
            logger.info('Price stream closed, reconnecting in 5s...')
            eventlet.sleep(5)
            run()  # reconnect

        def on_open(ws):
            logger.info(f'Price stream connected: {streams}')

        try:
            wsa = ws_lib.WebSocketApp(url, on_message=on_message,
...
# Warm up cache and start price stream on boot
start_cache_refresh()
...
def get_settings():
    data = {k: get_setting(k) for k in [
        'max_positions','stop_loss_pct','take_profit_pct',
        'position_size_usdt','trading_mode','active_pairs',
        'bot_running','starting_balance']}
    # Return masked keys so UI can show whether they are set
...
def upd_settings():
    ...
    # Save API keys — only update if not masked placeholder
...
def news_test():
    ...
    try:
        ...
        r = req.get('https://newsapi.org/v2/everything', timeout=15, params={
